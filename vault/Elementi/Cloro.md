...
La questione la chiude Humphry Davy nel 1810, e la chiude togliendo qualcosa invece di aggiungerlo. Prova in tutti i modi a estrarre ossigeno da quel gas, con il carbone incandescente e con i metalli più avidi di ossigeno che conosce, e non ne cava nulla: se l'ossigeno non c'è, il gas non è un ossido ma un elemento. Lo annuncia alla Royal Society il 15 novembre di quell'anno e lo chiama chlorine, dal greco chloros, verde-giallo, cioè dal suo colore e da nient'altro.
...
flowchart LR
    E["Cloro"]
    C1["Cloruro di sodio<br/>NaCl"]
    E --> C1
    U1(["sale da cucina"])
    C1 --> U1
    U2(["materia prima dell'industria cloro-soda"])
    C1 --> U2
    C2["Acido cloridrico<br/>HCl"]
    E --> C2
    U3(["decapaggio dei metalli"])
    C2 --> U3
    U4(["digestione gastrica"])
    C2 --> U4
    C3["Ipoclorito di sodio<br/>NaClO"]
    E --> C3
    U5(["candeggina e disinfettante"])
    C3 --> U5
    U6(["potabilizzazione delle acque"])
    C3 --> U6
    C4["Cloruro di polivinile (PVC)<br/>(C2H3Cl)n"]
    E --> C4
    U7(["tubature e infissi"])
    C4 --> U7
    U8(["guaine per cavi elettrici"])
    C4 --> U8
