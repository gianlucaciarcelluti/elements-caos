...
flowchart LR
    E["Zolfo"]
    C1["Acido solforico<br/>H2SO4"]
    E --> C1
    U1(["fertilizzanti fosfatici"])
    C1 --> U1
    U2(["raffinazione del petrolio"])
    C1 --> U2
    U3(["elettrolita delle batterie al piombo"])
    C1 --> U3
    C2["Anidride solforosa<br/>SO2"]
    E --> C2
    U4(["conservante del vino"])
    C2 --> U4
    U5(["sbiancamento della cellulosa"])
    C2 --> U5
    U6(["intermedio per l'acido solforico"])
    C2 --> U6
    C3["Solfuro di idrogeno<br/>H2S"]
    E --> C3
    U7(["sintesi di solfuri metallici"])
    C3 --> U7
    U8(["analisi chimica qualitativa"])
    C3 --> U8
    U9(["produzione di acqua pesante"])
    C3 --> U9
    C4["Pirite<br/>FeS2"]
    E --> C4
    U10(["fonte mineraria di zolfo"])
    C4 --> U10
    U11(["produzione di acido solforico"])
    C4 --> U11
    U12(["antiche pietre focaie"])
    C4 --> U12
