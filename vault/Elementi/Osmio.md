...
flowchart LR
    E["Osmio"]
    C1["Tetrossido di osmio<br/>OsO4"]
    E --> C1
    U1(["colorazione in microscopia elettronica"])
    C1 --> U1
    U2(["ossidazione degli alcheni"])
    C1 --> U2
    U3(["sintesi organica"])
    C1 --> U3
    C2["Osmato di potassio<br/>K2OsO4"]
    E --> C2
    U4(["reagente meno volatile per le diidrossilazioni"])
    C2 --> U4
    U5(["catalisi di laboratorio"])
    C2 --> U5
    C3["Diossido di osmio<br/>OsO2"]
    E --> C3
    U6(["rivestimenti conduttivi"])
    C3 --> U6
    U7(["intermedio di raffinazione"])
    C3 --> U7
