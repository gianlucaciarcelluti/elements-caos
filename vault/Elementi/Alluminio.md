...
flowchart LR
    E["Alluminio"]
    C1["Ossido di alluminio (allumina)<br/>Al2O3"]
    E --> C1
    U1(["materia prima per la produzione del metallo"])
    C1 --> U1
    U2(["abrasivi e refrattari"])
    C1 --> U2
    U3(["corindone, rubino e zaffiro"])
    C1 --> U3
    C2["Idrossido di alluminio<br/>Al(OH)3"]
    E --> C2
    U4(["antiacido gastrico"])
    C2 --> U4
    U5(["ritardante di fiamma"])
    C2 --> U5
    C3["Allume di potassio<br/>KAl(SO4)2·12H2O"]
    E --> C3
    U6(["mordente per tintura"])
    C3 --> U6
    U7(["chiarificante per le acque"])
    C3 --> U7
    U8(["emostatico"])
    C3 --> U8
    C4["Cloruro di alluminio<br/>AlCl3"]
    E --> C4
    U9(["catalizzatore nella sintesi organica"])
    C4 --> U9
