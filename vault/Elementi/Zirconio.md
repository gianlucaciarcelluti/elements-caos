...
anno_scoperta: 1789
anno_stimato: false
scopritori: [Martin Heinrich Klaproth]
epoca: Chimica pneumatica
categoria: metallo_di_transizione
gruppo: 4
periodo: 5
posizione_cronologica: 33
tempo_lettura: 5
ha_approfondimento: false
tags: [elemento, metallo-di-transizione, epoca/pneumatica, secolo/XVIII]
aliases: [Zr, Zirconium]
---
...
flowchart LR
    E["Zirconio"]
    C1["Silicato di zirconio (zircone)<br/>ZrSiO4"]
    E --> C1
    U1(["sabbie per fonderia"])
    C1 --> U1
    U2(["gemma"])
    C1 --> U2
    U3(["opacizzante per ceramiche"])
    C1 --> U3
    C2["Ossido di zirconio (zirconia)<br/>ZrO2"]
    E --> C2
    U4(["ceramiche tecniche e corone dentali"])
    C2 --> U4
    U5(["zirconia cubica come simulante del diamante"])
    C2 --> U5
    U6(["sensori di ossigeno"])
    C2 --> U6
    C3["Zircaloy (lega di zirconio)<br/>Zr-Sn-Fe-Cr"]
    E --> C3
    U7(["guaine per il combustibile nucleare"])
    C3 --> U7
    C4["Tetracloruro di zirconio<br/>ZrCl4"]
    E --> C4
    U8(["intermedio per la produzione del metallo"])
    C4 --> U8
    U9(["catalizzatore"])
    C4 --> U9
